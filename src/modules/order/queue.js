  ...
  async (job) => {
    ...
    try {
      ...
      for (const it of parsedBody.items) {
        // here check if any of the asked is not in db items
        const mi = menuById[it.menu_item_id];
        if (!mi.available)
          throw new AppError(`Menu item ${mi.name} is not available`, 400, "validation");
      }
      ...
      const interval = computeEstimatedReadyInterval(
        branchInfo.avg_waiting_time ?? 0,
        parsedBody.items.length
      );
      ...
      // save idempotency
      if (idempotencyKey) {
        const { id: _id, ...orderWithoutSerialId } = order;
