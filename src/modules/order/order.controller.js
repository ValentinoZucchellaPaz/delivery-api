...
async function changeStatusIfAllowed({
  public_order_id,
  actor,
  _allowedRoles,
  fromStatuses,
  ...
  if (actor.role === "customer") {
    ...
    if (!branchInfo) throw new NotFoundError("Branch not found");
    if (branchInfo.owner_id !== actor.user_id)
      throw new AppError("Forbidden", 403, "authorization");
  }
...
export async function markPaid(req, res, next) {
  try {
    ...
    const updated = await repo.updateOrderTimestampsAndStatus(id, {
      paid: true,
  ...
  try {
    ...
    if (req.user.role === "customer") {
      if (order.status !== "pending")
        throw new AppError("Customer can cancel only pending orders", 400, "state");
      const updated = await repo.updateOrderTimestampsAndStatus(id, {
        status: "cancelled",
    ...
    if (req.user.role === "restaurant_owner") {
      ...
      if (!["preparing", "pending"].includes(order.status))
        throw new AppError("Branch can cancel only pending or preparing orders", 400, "state");
      const updated = await repo.updateOrderTimestampsAndStatus(id, {
        status: "cancelled",
        cancelled_at: new Date().toISOString(),
      });
