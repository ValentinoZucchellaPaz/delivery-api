...
export async function updateBranch(req, res, next) {
  try {
    ...
    if (Object.keys(parsed).length == 0)
      throw new AppError("Must change something (address, city, avg_waiting_time)", 400, "params");
...
export async function editMenu(req, res, next) {
  ...
  try {
    ...
    if (
      ...
      parsed.delete.length == 0 &&
      parsed.add.length == 0
    )
      throw new AppError(
        "Must change something (name, add[item], delete[item]; item:{ name, description, price, available })",
        400,
        "params"
      );
